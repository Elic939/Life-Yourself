...
internal static class Ui
{
 ...
 public static TextBox Field(Panel p,string label,string value="",bool multiline=false){p.Children.Add(Text(label,12));var t=new TextBox{Text=value,AcceptsReturn=multiline,TextWrapping=multiline?TextWrapping.Wrap:TextWrapping.NoWrap,MinHeight=multiline?80:0};p.Children.Add(t);return t;}
 public static ComboBox Choice(Panel p,string label,string[] values,int selected=0){p.Children.Add(Text(label,12));var c=new ComboBox{ItemsSource=values,SelectedIndex=selected};p.Children.Add(c);return c;}
 public static CheckBox Check(Panel p,string label,bool value,Action<bool> change){var c=new CheckBox{Content=label,IsChecked=value,Margin=new Thickness(0,8,0,8)};c.Click+=(_,_)=>change(c.IsChecked==true);p.Children.Add(c);return c;}
