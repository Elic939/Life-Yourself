...
public partial class MainWindow
{
 ...
 void EditTask(DailyTask item)
 {
  StartEditor("日常事项");var title=Ui.Field(DrawerContent,"事项名称",item.Title);var date=Ui.Field(DrawerContent,"日期 yyyy-MM-dd",item.Date.ToString("yyyy-MM-dd"));var time=Ui.Field(DrawerContent,"时间 HH:mm（可留空）",item.Time?.ToString("HH:mm")??"");var priority=Ui.Choice(DrawerContent,"优先级",["普通","重要"],item.Priority=="important"?1:0);
